...
def begin_generate_thread(core, session_id):
    core.generate_save(session_id, wav=True, log=True)
...
def answer_gen():
    ...
    session.generate_midi_seg = construct_midi_seg(session_id + '/textured_chord_gen.mid', session.segmentation, session_id)
    ...
    return resp(session_id=session_id, more=[['log', new_log]])
